(ns kotodama.life
  "Organism life — the 'organic' axis. Mood (joucho 情緒) EMERGES from a fold over
  the actor's lived event history (replayable from the Datom log); the heartbeat
  cadence gates outward activity; the Stage-D prior-consensus fold lets an actor
  read its own priors. All pure, all generic — no domain knowledge. Each actor
  individuates from its own history, so a fleet is organically distinct, not clones."
...
(defn prior-shortcut?
  "Take a prior shortcut only when the consensus is strong, authorized, and
  grounded in matching input (≥3 outcomes, ≥800‰ confidence, ≥1 input match,
  dominant = \"authorized\")."
  [consensus]
